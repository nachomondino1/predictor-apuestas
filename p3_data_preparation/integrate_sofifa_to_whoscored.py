...
def player_data_in_match(df_jug_part, df_jug):
    ...
    try:  # cuidado que si mejoras la extraccion, el cambio puede que no se vea puesto que levanta el df_jug_encontrado viejo...
        df_jug_encontrados = pd.read_excel(f'/Users/nachomondino/Documents/GitHub/predictor-apuestas/p3_data_preparation/data/{pais}/df_integrated_jug_encontrados.xlsx')
    except:
        df_jug_encontrados = pd.DataFrame(columns=['id_jug', 'nombre', 'equipo', 'fecha', 'edad', 'altura', 'overall_rating', 'valor_mercado', 'str_encont'])
