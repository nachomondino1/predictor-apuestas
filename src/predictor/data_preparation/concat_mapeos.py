...
from predictor.utils.io import read_df, write_df
...
def concat_integrate_data_by_country(d_countries: dict, d_dates: dict, verbose: int = 0):
    ...
    for id_country in d_countries.keys():
        ...
        # Levanto su df_map
        df_player_sofifa_p = read_df(f'data/{country}/data_preparation/{date}/clean_data/df_player_sofifa_cleaned.xlsx')
        df_player_fifa_sofifa_p = read_df(f'data/{country}/data_preparation/{date}/clean_data/df_player_fifa_sofifa_cleaned.xlsx')
        df_map_p = pd.read_excel(f'data/{country}/data_preparation/{date}/integrate_data/df_map_players_fs_so.xlsx', index_col=0)
...
def integrate_with_new_map(l_countries):
    ...
    """
    from predictor.stages import DataPreparation
    dp = DataPreparation()
    ...
    for country in l_countries:
        ...
        df_match = read_df(f'./data/{country}/data_preparation/clean_data/df_match_cleaned.xlsx')
        df_match_player = read_df(f'./data/{country}/data_preparation/clean_data/df_match_player_cleaned.xlsx')
...
if __name__ == "__main__":
    ...
    base_path_clean = 'data/_shared/data_preparation/clean_data'
    base_path_int = 'data/_shared/data_preparation/integrate_data'
