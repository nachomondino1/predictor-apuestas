...
def collect_predictions(d_run: dict, l_countries:list, n_days:float, df_historial_predicciones:pd.DataFrame):
    ...
    df_predicciones.to_excel(f'p6_deployment/data/predicciones.xlsx', index=True)
    df_historial_predicciones.to_excel(f'p6_deployment/data/historial_predicciones.xlsx', index=True)
