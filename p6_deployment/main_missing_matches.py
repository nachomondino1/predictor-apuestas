...
from p2_data_understanding.collect_initial_data.scraper_flashscore import extract_data_faltante_flashcore


def collect_initial_data_faltante(id_country, country, export=True):
    """
    ...
    for id_competition in l_competencies:
        ...
        df_comp_filt = df_comp[(df_comp['id_country'] == id_country) & (df_comp['id_competition'] == id_competition)]  # Para extrar varios countryes?: df = df_comp[df_comp['country'].isin(l_countryes)]
        ...
        df_match_miss_matches, df_match_player_miss_matches = extract_data_faltante_flashcore(country, competition, list(df_match.index))
...
def main(id_country, country, var_resp):
    """
    ...
    df_match_missing, df_match_player_missing = collect_initial_data_faltante(id_country, country)
    ...
    df_match_missing, df_match_player_missing, df_player = dp.clean_data(df_match_missing, df_match_player_missing, df_player, thr_nan_col=1, export=False)  # thr_nan_col = 1  -> No quiero que elimine nada
    ...
    df = pd.read_excel(f'./p3_data_preparation/data/{country}/df_integrated.xlsx')  # Levanto datos viejos (para rellenarlos)
...
def prueba():
    ...
    if data_unders:
        ...
        # Collect missing data
        df_match_missing, df_match_player_missing = collect_initial_data_faltante(country, export=export)
        df_player = pd.read_excel(f'./p2_data_understanding/data/{country}/df_player.xlsx') # Podria recolectar nueva version del ultimo fifa.
