...
def create_schedule(df_next_matches, min_before_match=15):
    ...
    df.to_excel('p6_deployment/automatize_predict/update_predictions/schedules.xlsx', index=False)
    return df
  ...
  {job_name}:
    ...
    steps:
      ...
      - name: Run collect_data script
        run: |
          python p6_deployment/automatize_predict/collect_predictions.py 0.05 "[{id_country}]" '{{"run_missing": false, "data_unders": true, "data_prep": true, "modeling": true, "export": true}}'
...
if __name__ == "__main__":
    ...
    # Defino argumentos
    df_next_matches = pd.read_excel('data/historial_predicciones.xlsx') # Levantar proximos partidos --> tengo que garantizar que sean los partidos de los proximos 15 dias.
    ...
    # Creo schedules.xlsx
    df_schedules = create_schedule(df_next_matches) # pd.read_excel(file_path) # sheet_name='schedules' # file_path = 'github_actions/p6_deployment/automatize_predict/schedules.xlsx'
