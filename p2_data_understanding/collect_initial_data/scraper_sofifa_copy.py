...
class SofifaCrawler(Crawler):
    ...
    def select_fields(self, l_fields):
        xpath = "//form[@action='/?']//input"
        
        for field in l_fields:
            super().fill_form(xpath_input=xpath, text=field)
    ...
    def select_league_as_filter(self, country, league):
        ...
        xpath_input = './/form[@class="pjax-form" and (@action="/players" or @action="/teams")]//input[@placeholder="Leagues"]'
        input_league = super().extract_tag(xpath=xpath_input)
        input_league.send_keys(league)
        # super().fill_form(xpath_input=xpath, text=league, enter=False)

        # Selecciona la opcion segun name del country con send_keys
        ## Posibles leagues segun nuestra busqueda
        l_posibles_leagues = super().extract_tags(xpath=f'{xpath_input}//parent::div//following-sibling::div//div[starts-with(@class, "choices-item")]', sec_wait=self.SEC_WAIT_MAX)  # a veces crashea el click pero funciona
...
def determine_comps_to_extract(id_country):
    ...
    return df_comp_country

def extract_players(id_country):
    ...
    country = id_country_to_name(id_country)
    df_comp_country = determine_comps_to_extract(id_country)
    print(f' COUNTRY: {country} '.center(120, '#'), f"\nCompeticiones a extraer:\n{df_comp_country['competition_flashscore']}")

    # Ingreso a pagina de sofifa.com
    crawler.driver.get("https://sofifa.com")

    # POR COMPETITION
    for i, row in df_comp_country.iterrows():

        league = row["competition_flashscore"]
        league_form = league.lower().replace(" ", "-")
        print(f' Competition: {league} '.center(120, '+'))
        ...
        # Selecciono filtros los campos buscados (age, height, or, pot, valor_merc, etc)
        l_fields = ['ID', 'Height', 'Preferred foot']
        crawler.select_fields(l_fields)


        '''
        # Extraigo datos de players de Sofifa 
        ## Player
        df_player_sofifa, df_player_fifa_sofifa = extract_players(id_country, country, row['id_competition'], row['competition_sofifa'], export=True)
        ...
        df_player_sofifa_concat.to_excel(f'{BASE_DIR_LOCAL}/df_player_{country}_{row['competition_sofifa']}.xlsx', index=True)
        df_player_fifa_sofifa_concat.to_excel(f'{BASE_DIR_LOCAL}/df_player_fifa_{country}_{row['competition_sofifa']}.xlsx', index=False)
        '''
