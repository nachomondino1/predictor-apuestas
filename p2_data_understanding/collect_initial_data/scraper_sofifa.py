...
def update_player_data(id_country, n_seasons_update: int = 1, verbose: int = 1):
    """
    ...
    # Defino variables
    df_teams_sofifa, df_player_sofifa, df_player_fifa_sofifa = pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    base_path_updated = f'{base_path}/data_seg/updated'
    directories.make_directories(l_directorios=[base_path_updated])
    ...
    for i, row in df_comp_country.iterrows():
        ...
        ## Player
        df_player_sofifa_comp, df_player_fifa_sofifa_comp = extract_players(id_country, country, row['id_competition'], row['competition_sofifa'], n_seasons=n_seasons_update, export=True)
        ## Teams
        ...
        if verbose >= 1:
            df_player_sofifa.to_excel(f'{base_path_updated}/df_player_{country}_{row['competition_sofifa']}.xlsx', index=True)
            df_player_fifa_sofifa.to_excel(f'{base_path_updated}/df_player_fifa_{country}_{row['competition_sofifa']}.xlsx', index=False)
    
    # Exporto datos x seguridad
    if verbose >= 1:
        # Exporto datos a Desktop (x seguridad)
        df_player_sofifa.to_excel(f'{base_path_updated}/df_player_sofifa_{country}.xlsx', index=True)
        df_player_fifa_sofifa.to_excel(f'{base_path_updated}/df_player_fifa_sofifa_{country}.xlsx', index=True)
        # df_teams_sofifa_concat.to_excel(f'{base_path_updated}/df_teams_{country}.xlsx', index=True)

    # Concateno datos 1) viejos sin fifas actualziados + 2) datos recien extraidos
    df_player_sofifa_ct, df_player_fifa_sofifa_ct = replace_last_seasons(df_player_sofifa, df_player_fifa_sofifa, base_path=base_path)

    ## Exporto datos (ya listos para usar en main.py)
    df_player_sofifa_ct.to_excel(f'{base_path}/df_player_sofifa.xlsx', index=True)
    df_player_fifa_sofifa_ct.to_excel(f'{base_path}/df_player_fifa_sofifa.xlsx', index=True)

def replace_last_seasons(df_player_sofifa, df_player_fifa_sofifa, base_path, verbose: int = 1):
    """
    ...
    df_player_fifa_sofifa_old = pd.read_excel(f'{base_path}/df_player_fifa_sofifa.xlsx')
    if verbose >= 2:
        logger.info(df_player_sofifa_old)
        logger.info(df_player_fifa_sofifa_old)

    ## Mover los datos viejos en una carpeta nueva pues lo reemplazare
    base_path_updated_old = f'{base_path}/data_seg/updated/old'
    directories.mover_archivo(origen=f'{base_path}/df_player_sofifa.xlsx', destino=f'{base_path_updated_old}')
    directories.mover_archivo(origen=f'{base_path}/df_player_fifa_sofifa.xlsx', destino=f'{base_path_updated_old}')

    ## Determinar que fifas se actualizaron
    l_fifas_updated = df_player_fifa_sofifa['fifa'].unique()
    print(f"Fifas actualizados: {l_fifas_updated}")

    ## Elimino fifas actualizados en los datos viejos
    df_player_fifa_sofifa_old_filt = df_player_fifa_sofifa_old[~df_player_fifa_sofifa_old['fifa'].isin(l_fifas_updated)]
    l_ids_que_quedan = df_player_fifa_sofifa_old_filt['id_player'].unique()
    print(f'A: {df_player_fifa_sofifa_old.shape} --> {df_player_fifa_sofifa_old_filt.shape}')

    df_player_sofifa_old_filt = df_player_sofifa_old[df_player_sofifa_old.index.isin(l_ids_que_quedan)]
    print(f'B: {df_player_sofifa_old.shape} --> {df_player_sofifa_old_filt.shape}')

    ## Concateno datos
    df_player_sofifa_ct = pd.concat([df_player_sofifa, df_player_sofifa_old_filt], axis=0)  # Primero lo nuevo para que integre esos jugadores primero?
    df_player_fifa_sofifa_ct = pd.concat([df_player_fifa_sofifa, df_player_fifa_sofifa_old_filt], axis=0)
    print(f'C: {df_player_sofifa_ct.shape} y {df_player_fifa_sofifa_ct.shape}')
    return df_player_sofifa_ct, df_player_fifa_sofifa_ct
...
if __name__ == "__main__":
    ...
    update_player_data(id_country, n_seasons_update=n_seasons_to_extract, verbose=1)