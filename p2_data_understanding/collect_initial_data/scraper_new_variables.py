...
def extract_new_data(df, export: bool = True):
    """
    # Parameters:
        df: Dataframe 
    ...
    for id_match, row in df.iterrows():
        ...
        # Extract new_data
        d_row_match.update(crawler.extract_new_data())
        # d_row_match.update(crawler.extract_disclaimer_data()) # Penalties and estadio neutral
        print()
        print(d_row_match)
...
# Penalties and estadio neutral
def filter_matches_penalties(df):
...
if __name__ == "__main__":
    ...
    for id_country in l_countries:
        ...
        # Filtra df
        # df = filter_matches_penalties(df)
        # df = convert_goals_to_int(df)

        # Extraigo neutralidad y penalties
        df_match = extract_new_data(df)

        # Corrigo data
        # df = add_neutralidad(df, df_match)
        # df = corregir_goals(df, df_match)
        df_match.to_excel(f'data/{country}/p6_deployment/missing/old_updated/df_match_2.xlsx', index=True)
