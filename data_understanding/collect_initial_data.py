...
def extract_flashscore():
    ...
    df_comp = pd.read_excel(
        '/Users/nachomondino/Documents/GitHub/predictor-apuestas/data_understanding/collect_data/data/entidad_competicion.xlsx')
    ...
    for pais in df_comp['pais'].unique()[:1]:
        ...
        for competicion, categoria in zip(l_comp, l_cat):
            ...
            l_tag_temporadas = crawler.extract_tags(
                xpath='.//section[@id="tournament-page-archiv"]//div[@class="archive__row"]/div[@class="archive__season"]/a')
            ...
            for url_temp in l_urls_temporadas[4:5]:  # solo 2018/19
                ...
                crawler.click_boton(xpath='.//a[text()="Mostrar más partidos"]', sec_wait=SEC_WAIT_LONG * 3,
                                    repeat_click=True)  # Si hace click, es None. Si falla, es un str  # A veces me tira (y no entiendo por qué): selenium.common.exceptions.StaleElementReferenceException: Message: stale element reference: stale element not found

                # Extraigo partidos (items) y sus ids
                l_items = crawler.extract_tags(
                    xpath='.//div[@class="sportName soccer"]//div[@title="¡Haga click para detalles del partido!"]')
                ...
                for id in l_ids:
                    ...
                    cont_part += 1
                    print(
                        f" Partido {cont_part} de {len(l_items)}. Recolectado el {cont_part / len(l_items) * 100:.0f}% ".center(
                            120, "."))
                    ...
                    fecha_str = crawler.extract_tag(xpath='.//div[@class="duelParticipant__startTime"]', text=True,
                                                    sec_wait=SEC_WAIT_LONG)
                    ...
                    if fecha_dt < fecha_act:
                        ...
                        d_nueva_fila[
                            'fecha'] = fecha_dt  # d_nueva_fila['fecha'] = crawler.extract_tag(xpath='.//div[@class="duelParticipant__startTime"]', text=True, sec_wait=SEC_WAIT_LONG)
                        ...
                        d_nueva_fila['goles_loc'] = crawler.extract_tag(
                            xpath='.//div[@class="detailScore__wrapper"]/span[1]', text=True, sec_wait=SEC_WAIT)
                        d_nueva_fila['goles_vis'] = crawler.extract_tag(
                            xpath='.//div[@class="detailScore__wrapper"]/span[3]', text=True, sec_wait=SEC_WAIT)
                        ...
                        if crawler.click_boton(
                                xpath='.//div[@class="tabs tabs__detail--nav"]//a[text()="Estadísticas"]',
                                sec_wait=SEC_WAIT_LONG) is not False:

                            time.sleep(random.uniform(SEC_WAIT + 3,
                                                      SEC_WAIT_LONG + 3))  # Falla el campo posesion_loc puesto que es el primero en ser extraido y aun no cargo...
                            ...
                            for estadistica in d_estadisticas.keys():
                                ...
                                d_nueva_fila[f'{estadistica}_loc'] = crawler.extract_tag(
                                    xpath=f'.//div[text()="{d_estadisticas[estadistica]}"]//preceding-sibling::div',
                                    text=True,
                                    sec_wait=SEC_WAIT)  # Falla el campo posesion_loc puesto que es el primero en ser extraido y aun no cargo...
                                d_nueva_fila[f'{estadistica}_vis'] = crawler.extract_tag(
                                    xpath=f'.//div[text()="{d_estadisticas[estadistica]}"]//following-sibling::div',
                                    text=True, sec_wait=SEC_WAIT)

                        # Extraigo campos de hoja "Formaciones"
                        if crawler.click_boton(xpath='.//div[@class="tabs tabs__detail--nav"]//a[text()="Formaciones"]',
                                               sec_wait=SEC_WAIT) is not False:

                            time.sleep(random.uniform(SEC_WAIT + 3,
                                                      SEC_WAIT_LONG + 3))  # Por posible falla en el primer campo a extraer

                            # Por seccion ("Formacion inicial", "Suplentes" y  "Ausentes")
                            for formacion in d_formaciones.keys():

                                # Si existe la seccion "Formaciones"
                                if crawler.extract_tag(xpath=f'.//div[text()="{formacion}"]',
                                                       sec_wait=SEC_WAIT_LONG) is not None:

                                    # Extraigo listado de jugadores
                                    l_tags_jug_loc = crawler.extract_tags(
                                        xpath=f'.//div[text()="{formacion}"]//following-sibling::div//div[@class="lf__side"][1]//*[@class="lf__participantName"]',
                                        sec_wait=SEC_WAIT * 2)  # Es lista de tags o None
                                    l_tags_jug_vis = crawler.extract_tags(
                                        xpath=f'.//div[text()="{formacion}"]//following-sibling::div//div[@class="lf__side"][2]//*[@class="lf__participantName"]',
                                        sec_wait=SEC_WAIT * 2)  # Es lista de tags o None
                                    ...
                                    if l_tags_jug_loc is not None:
                                        for i in range(len(l_tags_jug_loc)):
                                            ...
                                            # Extraigo nombre
                                            d_nueva_fila[
                                                f'jug_{d_formaciones[formacion]}_loc_{i + 1}'] = nombre  # l_tags_jug_loc[i].text
                                    ...
                                    if l_tags_jug_vis is not None:
                                        for i in range(len(l_tags_jug_vis)):
                                            # Intento extraer nombre completo (solo si tiene link asociado) (e.g. "genez nahuel")
                                            try:
                                                url_with_nombre = l_tags_jug_vis[i].get_attribute(
                                                    'href')  # href="https:/www.flashscore.com.ar/jugador/genez-nahuel/WtErcTOs/"
                                                nombre = url_with_nombre.split('/')[4].replace('-', " ")
                                            # Extraigo nombre corto (e.g. "N.genez")
                                            except:
                                            ...
                                            d_nueva_fila[
                                                f'jug_{d_formaciones[formacion]}_vis_{i + 1}'] = nombre  # l_tags_jug_vis[i].text
                            ...
                            # Intento extraer nombre completo (solo si tiene link asociado) (e.g. "genez nahuel")
                            try:
                                url_with_nombre = crawler.extract_tag(
                                    xpath='.//div[text()="Entrenadores"]//following-sibling::div//div[@class="lf__side"][1]//a[@class="lf__participantName"]',
                                    attribute='href',
                                    sec_wait=SEC_WAIT)  # href="https:/www.flashscore.com.ar/jugador/genez-nahuel/WtErcTOs/"
                                d_nueva_fila['dt_loc'] = url_with_nombre.split('/')[4].replace('-', " ")
                            # Extraigo nombre corto (e.g. "N.genez")
                            except:
                                d_nueva_fila['dt_loc'] = crawler.extract_tag(
                                    xpath='.//div[text()="Entrenadores"]//following-sibling::div//div[@class="lf__side"][1]',
                                    text=True, sec_wait=SEC_WAIT)

                            # Intento extraer nombre completo (solo si tiene link asociado) (e.g. "genez nahuel")
                            try:
                                url_with_nombre = crawler.extract_tag(
                                    xpath='.//div[text()="Entrenadores"]//following-sibling::div//div[@class="lf__side"][2]//a[@class="lf__participantName"]',
                                    attribute='href',
                                    sec_wait=SEC_WAIT)  # href="https:/www.flashscore.com.ar/jugador/genez-nahuel/WtErcTOs/"
                                d_nueva_fila['dt_vis'] = url_with_nombre.split('/')[4].replace('-', " ")
                            # Extraigo nombre corto (e.g. "N.genez")
                            except:
                                d_nueva_fila['dt_vis'] = crawler.extract_tag(
                                    xpath='.//div[text()="Entrenadores"]//following-sibling::div//div[@class="lf__side"][2]',
                                    text=True, sec_wait=SEC_WAIT)
                ...
                df_part.to_excel(
                    f'/Users/nachomondino/Documents/GitHub/predictor-apuestas/data_understanding/collect_data/data/data_seg/{competicion_form}_{temp_year.replace("/", "_")}_{pais}.xlsx',
                    index=False)

        # Guardado de archivo excel en computadora
        df_part.to_excel(
            f'/Users/nachomondino/Documents/GitHub/predictor-apuestas/data_understanding/collect_data/data/{pais}/entidad_partido_prueba.xlsx',
            index=False)
