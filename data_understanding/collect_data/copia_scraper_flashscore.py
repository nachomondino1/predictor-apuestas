...
def delete_accent(text):  # Para borrar acentos en df_comp?
    """
    ...
    for pais in df_comp['pais'].unique():
        ...
        if pais == "Uruguay":
            ...
            try:
                ...
                df_part = pd.DataFrame(columns=['id'])  # inicializo con id para eviitar error en if id not in df_part
                        ...
                        if id not in list(df_part['id']):
                            ...
                            if crawler.click_boton(xpath='.//div[@class="tabs tabs__detail--nav"]//a[text()="Formaciones"]', sec_wait=SEC_WAIT) is not False:
                                ...
                                if crawler.extract_tag(xpath='.//div[text()="Formaciones iniciales"]', sec_wait=SEC_WAIT_LONG) is not None:
                                    ...
                                    # d_nueva_fila['l_jug_tit_loc'] = [tag.text for tag in l_tags_jug_tit_loc] if l_tags_jug_tit_loc is not None else []
                                    # d_nueva_fila['l_jug_tit_vis'] = [tag.text for tag in l_tags_jug_tit_vis] if l_tags_jug_tit_vis is not None else []
                                ...
                                if crawler.extract_tag(xpath='.//div[text()="Suplentes"]', sec_wait=SEC_WAIT) is not None:
                                    ...
                                    # d_nueva_fila['l_jug_sup_loc'] = [tag.text for tag in l_tags_jug_sup_loc] if l_tags_jug_sup_loc is not None else []
                                    # d_nueva_fila['l_jug_sup_vis'] = [tag.text for tag in l_tags_jug_sup_vis] if l_tags_jug_sup_vis is not None else []
                                ...
                                if crawler.extract_tag(xpath='.//div[text()="Jugadores ausentes"]', sec_wait=SEC_WAIT) is not None:
                                    ...
                                    # d_nueva_fila['l_jug_ausentes_loc'] = [tag.text for tag in l_tags_jug_ausentes_loc] if l_tags_jug_ausentes_loc is not None else []
                                    # d_nueva_fila['l_jug_ausentes_vis'] = [tag.text for tag in l_tags_jug_ausentes_vis] if l_tags_jug_ausentes_vis is not None else []
