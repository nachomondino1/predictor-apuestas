...
def find_best_hiperparameters(var_resp, var_pred, pais):
    ...
    best_accuracy = 0.0
    cont_iter = 1
    l_modelos = [RandomForestClassifier(), xgb.XGBClassifier(), LogisticRegression(), SVC(), MLPClassifier()]  # Dejo solo los que tardan poco  # GradientBoostingClassifier()
    ...
    d_params = {
        ...
        'construct': {
            ...
            'n_anios_historial': [3]  # Nº años para determinar historial entre equipos
        },
        'clean': {
            'thr_nan_col': [None]  # , # Gano 0.2 # Porcentaje de NaN values maximo para las columnas
        },
        'select': {
            'thr_corr': [0.8, 0.5, None], # Correlacion minima para considerar correlacion entre variables
            'thr_fs': [0.4, 0.2, None]  # Peso minimo de una variable para ser considerada como importante [0-1] (siendo 1 el peso de la variable mas importante y 0 la menos)
        },
        'modeling': {
            ...
            'with_pca': [True],  # True lo aplica solo si conviene aplicar PCA()
    ...
    # Por combinacion de parametros de integrate_data
    for x, param_values_1 in enumerate(product(*d_params['integrate'].values()), start=1):
        ...
        for i, param_values_2 in enumerate(product(*d_params['construct'].values()), start=1):
            ...
            for m, param_values_3 in enumerate(product(*d_params['clean'].values()), start=1):
                ...
                print(f" Iter Nº {x}.{i}.{m} ".center(120, "#"))
                ...
                for j, param_values_4 in enumerate(product(*d_params['select'].values()), start=1):
                    ...
                    print(f" Iter Nº {x}.{i}.{m}.{j} ".center(120, "#"))
